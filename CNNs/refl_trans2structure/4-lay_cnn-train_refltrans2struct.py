...
def define_model(x):
    ...
    #define the model
    model = Model([rpin,rsin,tpin,tsin],[thout,mout1,mout2,mout3,mout4])
...
model.compile(optimizer='adam',loss=['mse','categorical_crossentropy','categorical_crossentropy','categorical_crossentropy','categorical_crossentropy'],metrics = ['mse','accuracy'],loss_weights=[2,1,1,1,1])
    ...
    modname = dr+'general_4lay4matinverse_model_v-tma_convolution_rprstpts_'+str(st)+'step_'+date.strftime("%Y")+date.strftime("%m")+date.strftime("%d")
...
def fit_fcn(x):
    ...
    history = model.fit([tr_rp,tr_rs,tr_tp,tr_ts],[thr,m1r,m2r,m3r,m4r],epochs=leng,batch_size=128,verbose=2,validation_data = ([va_rp,va_rs,va_tp,va_ts],[thv,m1v,m2v,m3v,m4v]),callbacks=[lr])

    #save model for later uses
    model.save(modname)

    #evaluate and print stats on the model fit to the log file
    (te_loss) = model.evaluate([te_rp,te_rs,te_tp,te_ts],[the,m1e,m2e,m3e,m4e])
    print(te_loss)
...
hparams = np.array([3,5,421,865,193,0.013620861219329,0.004935445927478])
