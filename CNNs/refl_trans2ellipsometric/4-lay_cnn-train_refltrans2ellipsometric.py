...
def split_data(m1,m2,m3,m4,th,ang,rp,rs,tp,ts,psi,delta,mn,mx):
    tr_m1=m1[mn:mx,:]
    tr_m2=m2[mn:mx,:]
    tr_m3=m3[mn:mx,:]
    tr_m4=m4[mn:mx,:]
    ...
    tr_tp=tp[mn:mx,:]
    tr_ts=ts[mn:mx,:]
    return (tr_m1,tr_m2,tr_m3,tr_m4,tr_th,tr_ang,tr_rp,tr_rs,tr_tp,tr_ts,tr_p,tr_d)
...
num_lay = 4
num_wave = 200
(ml1,ml2,ml3,ml4,ml5,th,ang,rp,rs,tp,ts,psi,delta) = readin_data('/home/arl92/Documents/newdata/data_rte_gen4lay5mat_0ge_240000n_v-tma_20201112.h5',num_mat,num_ang,num_lay,num_wave)

#directort to save data
dr = '/home/arl92/Documents/newdata/rt2rt/'
...
(m1r,m2r,m3r,m4r,thr,angr,rpr,rsr,tpr,tsr,psir,deltar) = split_data(ml1,ml2,ml3,ml4,th,ang,rp,rs,tp,ts,psi,delta,0,tr)
(m1v,m2v,m3v,m4v,thv,angv,rpv,rsv,tpv,tsv,psiv,deltav) = split_data(ml1,ml2,ml3,ml4,th,ang,rp,rs,tp,ts,psi,delta,tr,va)
(m1e,m2e,m3e,m4e,the,ange,rpe,rse,tpee,tse,psie,deltae) = split_data(ml1,ml2,ml3,ml4,th,ang,rp,rs,tp,ts,psi,delta,va,te)
...
def define_model(x):
    ...
    #indepent layers for individual spectra
    m = Dense(np2,activation='relu')(a)
    t = Dense(np2,activation='relu')(a)
    m = Dense(600,activation='relu')(m)
    t = Dense(600,activation='relu')(t)
    #output nodes
    m = Dense(600,activation=None)(m)
    t = Dense(600,activation=None)(t)


    #define the model
    model = Model([rpin,rsin,tpin,tsin],[m,t])
    ...
    model.compile(optimizer='adam',loss=['mse','mse'],metrics = ['mse'],loss_weights=[1,1])
    model.summary()

    #model name can be anything you want
    modname = dr+'general_4lay4matinverse_model_v-tma_convolution_rt2pd_'+str(st)+'step_'+date.strftime("%Y")+date.strftime("%m")+date.strftime("%d")
...
def fit_fcn(x):
    ...
    history = model.fit([tr_rp,tr_rs,tr_tp,tr_ts],[psir,deltar],epochs=leng,batch_size=128,verbose=2,validation_data = ([va_rp,va_rs,va_tp,va_ts],[psiv,deltav]),callbacks=[lr])

    #save model for later uses
    model.save(modname)

    #evaluate and print stats on the model fit to the log file
    (te_loss) = model.evaluate([te_rp,te_rs,te_tp,te_ts],[psie,deltae])
    print(te_loss)
...
hparams = np.array([3,1,643,1943,854,0.001563064118475,0.00675793278643])
