...
num_lay = 4
num_wave = 200
(ml1,ml2,ml3,ml4,th,ang,rp,rs,tp,ts,psi,delta) = readin_data('/home/arl92/Documents/newdata/data_rte_gen4lay5mat_0ge_240000n_v-tma_20201112.h5',num_mat,num_ang,num_lay,num_wave)

#directort to save data
dr = '/home/arl92/Documents/newdata/pd2mt/'
...
def define_model(x):
    ...
    thout = Dense(4,activation=None)(t)
    ...
    #define the model
    model = Model([pin,din],[thout,mout1,mout2,mout3,mout4])
    ...
    model.compile(optimizer='adam',loss=['mse','categorical_crossentropy','categorical_crossentropy','categorical_crossentropy','categorical_crossentropy'],metrics = ['mse','accuracy'],loss_weights=[2,1,1,1,1])
    model.summary()

    #model name can be anything you want
    modname = dr+'general_4lay4matinverse_model_v-tma_convolution_pd_'+str(st)+'step_'+date.strftime("%Y")+date.strftime("%m")+date.strftime("%d")
...
def fit_fcn(x):
    ...
    history = model.fit([tr_p,tr_d],[thr,m1r,m2r,m3r,m4r],epochs=leng,batch_size=128,verbose=2,validation_data = ([va_p,va_d],[thv,m1v,m2v,m3v,m4v]),callbacks=[lr])

    #save model for later uses
    model.save(modname)

    #evaluate and print stats on the model fit to the log file
    (te_loss) = model.evaluate([te_p,te_d],[the,m1e,m2e,m3e,m4e])
    print(te_loss)
...
hparams = np.array([0,1,233,751,503,0.001265711629074,0.006858104105641])
