...
def define_model(x):
    ...
    #define the model
    model = Model([pin,din],[thout,mout1,mout2,mout3,mout4,mout5])
    ...
    modname = dr+'general_5lay4matinverse_model_v-tma_convolution_pd_'+str(st)+'step_'+date.strftime("%Y")+date.strftime("%m")+date.strftime("%d")
...
def fit_fcn(x):
    ...
    history = model.fit([tr_p,tr_d],[thr,m1r,m2r,m3r,m4r,m5r],epochs=leng,batch_size=128,verbose=2,validation_data = ([va_p,va_d],[thv,m1v,m2v,m3v,m4v,m5v]),callbacks=[lr])

    #save model for later uses
    model.save(modname)

    #evaluate and print stats on the model fit to the log file
    (te_loss) = model.evaluate([te_p,te_d],[the,m1e,m2e,m3e,m4e,m5e])
    print(te_loss)
...
hparams = np.array([0,1,697,512,886,0.012870268837726,0.000549107622637])
