...
num_lay = 3
num_wave = 200
(ml1,ml2,ml3,th,ang,rp,rs,tp,ts,psi,delta) = readin_data('/home/arl92/Documents/newdata/data_rte_gen3lay5mat_0ge_240000n_v-tma_20201112.h5',num_mat,num_ang,num_lay,num_wave)
...
def define_model(x):
    ...
    #define the model
    model = Model([pin,din],[thout,mout1,mout2,mout3])
    ...
    modname = dr+'general_3lay4matinverse_model_v-tma_convolution_pd_'+str(st)+'step_'+date.strftime("%Y")+date.strftime("%m")+date.strftime("%d")
...
def fit_fcn(x):
    ...
    history = model.fit([tr_p,tr_d],[thr,m1r,m2r,m3r],epochs=leng,batch_size=128,verbose=2,validation_data = ([va_p,va_d],[thv,m1v,m2v,m3v]),callbacks=[lr])

    #save model for later uses
    model.save(modname)

    #evaluate and print stats on the model fit to the log file
    (te_loss) = model.evaluate([te_p,te_d],[the,m1e,m2e,m3e])
    print(te_loss)
...
hparams = np.array([0,2,392,945,394,0.00105478059863,0.000518131977959])
