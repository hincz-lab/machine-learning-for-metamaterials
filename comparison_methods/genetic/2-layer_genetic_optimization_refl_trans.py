...
filename = dr+'data_rte_gen2lay5mat_0ge_240000n_v-tma_20201112.h5'
(tm1,tm2,tth,tang,trp,trs,ttp,tts,psi,delta) = readin_data(filename,num_mat,num_ang,num_lay,num_wave)
...
filename = dr_save+'genetic_fitresults_2l5m_rprs_'+str(offset)+'-'+str(offset+sample)+'_'+'pop-'+str(solPerPop)+'gen-'+str(numGen)+'ens-'+str(ensemble)+'_'+start.strftime('%y')+start.strftime('%m')+start.strftime('%d')+'_'+start.strftime('%H')+start.strftime('%M')+start.strftime('%S')
print(filename)
...
tm = np.transpose(np.array([np.argmax(tm1[offset:sample+offset,:],axis=-1),np.argmax(tm2[offset:sample+offset,:],axis=-1)]))
