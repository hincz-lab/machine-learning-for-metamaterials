...
def crossover(parents,offSize):
    ...
    #since there is only one layer the layer is inhereted from a single parent
    for i in range(offSize[0]):
        p1 = (2*i-1)%parents.shape[0]
        off[i,:num_lay] = parents[p1,:num_lay]
        off[i,num_lay:] = parents[p1,num_lay:]
    return off
...
(tm1,tth,tang,trp,trs,ttp,tts,psi,delta) = readin_data(filename,num_mat,num_ang,num_lay,num_wave)
...
filename = dr_save+'genetic_fitresults_1l5m_rprs_'+str(offset)+'-'+str(offset+sample)+'_'+'pop-'+str(solPerPop)+'gen-'+str(numGen)+'ens-'+str(ensemble)+'_'+start.strftime('%y')+start.strftime('%m')+start.strftime('%d')+'_'+start.strftime('%H')+start.strftime('%M')+start.strftime('%S')
print(filename)
...
tm = np.transpose(np.array([np.argmax(tm1[offset:sample+offset,:],axis=-1)]))
