...
num_lay = 1
num_mat = 5
max_its = 1000 #optimization can be cut off for long processes, reduces outliers in solution time
num_wave = wave.size
...
popSize = (solPerPop,numParams)
numMating = np.int(solPerPop/2)
ensemble_fcn(solPerPop,psi[2,:],delta[2,:],ensemble,2)
...
filename = dr_save+'genetic_fitresults_1l5m_'+str(offset)+'-'+str(offset+sample)+'_'+'pop-'+str(solPerPop)+'gen-'+str(numGen)+'ens-'+str(ensemble)+'_'+start.strftime('%y')+start.strftime('%m')+start.strftime('%d')+'_'+start.strftime('%H')+start.strftime('%M')+start.strftime('%S')
print(filename)
