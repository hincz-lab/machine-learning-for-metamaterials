...
def readin_data(filename,nmat,nang,nlay,nwave):
    ...
    delta = (arrd[:,itl:])
    f.close()
    return (ml1,ml2,ml3,ml4,th,theta,rp,rs,tp,ts,psi,delta)
...
#the optimization is only performed here over reflectance spectra and not transmittance spectra
#in testing this performs a faster optimization coming to more accurate solutions. This could be due to relatively low transmission in a wide range of strcutures with 'thick' metal layers.
#Transmittance can be included by uncommenting the lines below and adding the additional terms to the concatenate statement in the output
@jit(nopython=True)
def residuals_fcn_rt(x,n,rp,rs,tp,ts):
    x = transform(x)
    ang = np.array([25.,45.,65.])
    n_rp = np.zeros(wave.size*ang.size,dtype=np.float64)
    n_rs = np.zeros(wave.size*ang.size,dtype=np.float64)
    n_tp = np.zeros(wave.size*ang.size,dtype=np.float64)
    n_ts = np.zeros(wave.size*ang.size,dtype=np.float64)
...
#             n_ts[i+wave.size*j] = tmm.trans_amp(0,ang[j], wave[i], n[:,i], x, n_super, n_subst[i])
    return np.concatenate(((n_rp-rp),(n_rs-rs)))#,(n_tp-tp),(n_ts-ts)))
...
@jit(nopython=True)
def calcFit(newPop,rp,rs,tp,ts):
    ...
    for i in range(newPop.shape[0]):
        ...
        mse = -1*np.mean(np.square(residuals_fcn_rt(l,n,rp,rs,tp,ts)))
        fit[i] = mse
...
@jit(nopython=True)
def history(solPerPop,rp,rs,tp,ts):
    ...
    for gen in range(numGen):
        ...
        fit = calcFit(newPop,rp,rs,tp,ts)
...
@jit(nopython=True)
def gen_fcn(rp,rs,tp,ts,solPerPop):
    (hist,optPop) = history(solPerPop,rp,rs,tp,ts)
    optFit = calcFit(optPop,rp,rs,tp,ts)
...
def ensemble_fcn(solPerPop,rp,rs,tp,ts,ensemble,g):
    ...
    try:
        ...
        for k in range(ensemble):
            results[k,:] = gen_fcn(rp,rs,tp,ts,solPerPop)
...
filename = dr+'data_rte_gen4lay5mat_0ge_240000n_v-tma_20201112.h5'
(tm1,tm2,tm3,tm4,tth,tang,trp,trs,ttp,tts,psi,delta) = readin_data(filename,num_mat,num_ang,num_lay,num_wave)
...
start = datetime.datetime.now()
results = Parallel(n_jobs=cores,verbose=10)(delayed(ensemble_fcn)(solPerPop,trp[g,:],trs[g,:],ttp[g,:],tts[g,:],ensemble,g) for g in range(offset,sample+offset))
end = datetime.datetime.now()
...
filename = dr_save+'genetic_fitresults_4l5m_rprs_'+str(offset)+'-'+str(offset+sample)+'_'+'pop-'+str(solPerPop)+'gen-'+str(numGen)+'ens-'+str(ensemble)+'_'+start.strftime('%y')+start.strftime('%m')+start.strftime('%d')+'_'+start.strftime('%H')+start.strftime('%M')+start.strftime('%S')
print(filename)
...
tm = np.transpose(np.array([np.argmax(tm1[offset:sample+offset,:],axis=-1),np.argmax(tm2[offset:sample+offset,:],axis=-1),np.argmax(tm3[offset:sample+offset,:],axis=-1),np.argmax(tm4[offset:sample+offset,:],axis=-1)]))
