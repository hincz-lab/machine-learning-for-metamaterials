...
for i in range(len(ftype)):
    ...
    filename = dr_save + 'lsq_ellips_fitresults_5l5m_type'+str(ftype[i])+str(nglob[i])+'_'+sta.strftime('%y')+sta.strftime('%m')+sta.strftime('%d')+'_'+sta.strftime('%H')+sta.strftime('%M')+sta.strftime('%S')
