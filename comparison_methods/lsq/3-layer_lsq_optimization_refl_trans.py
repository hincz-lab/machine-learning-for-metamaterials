            ...
            for j in np.delete(mat,i):
        	    ...
        	    #define the structure with chosen materials
            	    ...
            	    if mse <= bmse and np.all(plsq.x >= trange[0]) and np.all(plsq.x <= trange[1]):
                        bmse = mse
                        bx0 = np.concatenate((np.array([i,j,k]),plsq.x))
...
(tm1,tm2,tm3,tth,tang,trp,trs,ttp,tts,tp,td) = readin_data(filename,num_mat,num_ang,num_lay,num_wave)
...
for i in range(len(ftype)):
    ...
    metrics = accutest(np.transpose(np.array([np.argmax(tm1[start:start+systems,:],axis=1),np.argmax(tm2[start:start+systems,:],axis=1),np.argmax(tm3[start:start+systems,:],axis=1)])),tth[start:start+systems,:],results[:,:num_lay],results[:,num_lay:-1])
    print('Resutls:',metrics)
    #filename can be whatever you want
    #saves to the directory indicated with dr_save
    filename = dr_save + 'lsq_rprstpts_fitresults_3l5m_type'+str(ftype[i])+str(nglob[i])+'_'+sta.strftime('%y')+sta.strftime('%m')+sta.strftime('%d')+'_'+sta.strftime('%H')+sta.strftime('%M')+sta.strftime('%S')
